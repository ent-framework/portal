...
public class ${entity.name}Clp extends BaseModelImpl<${entity.name}> implements ${entity.name} {
	...
	<#if entity.isExtend()>
		...
		private ${entity.getParent().getPackagePath()}.model.${entity.getParent().getName()} _parent;
		@Override
		public ${entity.getParent().getPackagePath()}.model.${entity.getParent().getName()} getParent() throws PortalException, SystemException{
			if (_parent==null) {
				_parent = ${entity.getParent().getPackagePath()}.service.${entity.getParent().getName()}LocalServiceUtil.get${entity.getParent().getName()}(getPrimaryKey());
			}
			return _parent;
		}	
	</#if>
