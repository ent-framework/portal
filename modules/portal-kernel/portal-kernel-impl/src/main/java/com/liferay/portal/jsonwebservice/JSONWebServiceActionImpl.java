...
import com.liferay.portal.kernel.jsonwebservice.cache.WSCacheable;
import com.liferay.portal.kernel.jsonwebservice.cache.WebServiceCacheUtil;
...
public class JSONWebServiceActionImpl implements JSONWebServiceAction {
	...
	private Object _invokeActionMethod() throws Exception {
		...
		WSCacheable wsCacheable = AnnotationLocator.locate(actionMethod, actionObject.getClass(), WSCacheable.class);
		...
		if (wsCacheable!=null){
			...
			String key = actionObject.getClass().getName()+"."+actionMethod.getName();
			
			Object[] args = null;
			String name = PrincipalThreadLocal.getName();
			
			if (wsCacheable.dataByUser() && Validator.isNotNull(name)) {
				args = new Object[parameters.length+1];
				System.arraycopy(parameters, 0, args, 0, parameters.length);
				args[parameters.length] = name;
			} else {
				args = ArrayUtil.clone(parameters);
			}
			
			boolean forceRefresh = false;
			ServiceContext sc = _jsonWebServiceActionParameters.getServiceContext();
			if (sc!=null && sc.getRequest()!=null) {
				forceRefresh = sc.getRequest().getHeader("FORCE_REFRESH_CACHE")!=null;
			}
			
			if (forceRefresh) {
				result = actionMethod.invoke(actionObject, parameters);

				WebServiceCacheUtil.putResult(key, args, result, wsCacheable.value());
			...
			result = WebServiceCacheUtil.getResult(key, args);
