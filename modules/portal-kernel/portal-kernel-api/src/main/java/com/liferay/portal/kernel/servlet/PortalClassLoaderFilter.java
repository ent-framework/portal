...
public class PortalClassLoaderFilter
	...
	public void doFilter(
		...
		try {
			...
			invokerFilterChain.addFilterMapping(new FilterMapping(_filterName, _filterClass, _filter, _filterConfig,  Collections.EMPTY_LIST, Collections.EMPTY_LIST));
	...
	protected void doPortalInit() throws Exception {
		...
		_filterClass = _filterConfig.getInitParameter("filter-class");
		_filterName =  _filterConfig.getInitParameter("filter-name");
		
		if (_filterClass.startsWith("com.liferay.filters.")) {
			_filterClass = StringUtil.replace(
				_filterClass, "com.liferay.filters.",
				"com.liferay.portal.servlet.filters.");
		}

		_filter = (Filter)InstanceFactory.newInstance(classLoader, _filterClass);
