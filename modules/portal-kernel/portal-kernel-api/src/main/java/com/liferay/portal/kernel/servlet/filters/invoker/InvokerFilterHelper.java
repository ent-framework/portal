...
public class InvokerFilterHelper {
	...
	public void registerFilterScope(String filterName, String filterClassName, String contextPath) {
		if (_log.isDebugEnabled()) {
			_log.debug("initial filter class to DB, className :" + filterClassName + ", filter name :" + filterName + ", contextPath:" + contextPath);
		}
	...
	/**
	 * 
	 * create invoker filter chain base on uri match & filter site scoped
	 * 
	 * 
	 * @param request
	 * @param dispatcher
	...
	protected InvokerFilterChain createInvokerFilterChain(
		...
		for (FilterMapping filterMapping : _filterMappings) {
			...
			if (filterMapping.isMatch(request, dispatcher, uri)) {
				...
				invokerFilterChain.addFilterMapping(filterMapping);
			}
		}
	...
	protected void initFilter(
		...
		registerFilterScope(filterName, filterClassName, contextPath);
	...
	protected void initFilterMapping(
		String filterName, String filterClassName ,List<String> urlPatterns, List<String> dispatchers) {
		...
		FilterMapping filterMapping = new FilterMapping(filterName ,filterClassName ,filter, filterConfig, urlPatterns, dispatchers);
	...
	protected void readLiferayFilterWebXML(
		...
		Document document = UnsecureSAXReaderUtil.read(inputStream, false);

		Element rootElement = document.getRootElement();

		List<Element> filterElements = rootElement.elements("filter");

		Map<String,String> filterMappings = new HashMap<String,String>();
		...
		for (Element filterElement : filterElements) {
			...
			filterMappings.put(filterName, filterClassName);
		...
		for (Element filterMappingElement : filterMappingElements) {
			...
			initFilterMapping(filterName, filterMappings.get(filterName), urlPatterns, dispatchers);
		}
	}
