...
public class SolrIndexer implements Runnable {
	...
	public SolrIndexer(long companyId) {
	...
	@Override
	public void run() {
		reindex(GetterUtil.getInteger(PropsUtil.get(PropsKeys.INDEX_ON_STARTUP_DELAY)));
	}
	...
	protected void doReIndex(int delay) {
		...
		try {
			...
			if (_log.isInfoEnabled()) {
				_log.info("Reindexing solr failed");
			}
		}
	...
	private static final Logger _log = LoggerFactory.getLogger(SolrIndexer.class);
